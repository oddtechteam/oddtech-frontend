...
const Notification = ({ message, type, onClose }) => {
  ...
  return (
    <div className={`fixed bottom-4 right-4 ${bgColor} text-white px-4 py-2 rounded shadow-lg flex items-center justify-between min-w-[300px]`}>
      <span>{message}</span>
      <button onClick={onClose} className="ml-4 text-white hover:text-gray-200 focus:outline-none" aria-label="Close notification">×</button>
    </div>
  );
};
...
const AdminDashboard = () => {
  ...
  const [startDateRange, setStartDateRange] = useState({ startDate: null, endDate: null, key: 'selection' });
  const [dueDateRange, setDueDateRange] = useState({ startDate: null, endDate: null, key: 'selection' });
  ...
  useEffect(() => { if (allTasks.length > 0) applyFilters(); }, [startDateRange, dueDateRange, searchQuery, clientFilter]);
  ...
  const handleSearch = async (e) => {
    ...
    setSearchQuery(term);
    if (term.trim() === '') return setTasks(allTasks);

    try {
      setIsLoading(true);
      const results = await AdminService.searchTask(term);
      setTasks(results.length ? results : allTasks.filter(task => Object.values(task).some(val => String(val).toLowerCase().includes(term.toLowerCase()))));
    } catch {
      setTasks(allTasks.filter(task => Object.values(task).some(val => String(val).toLowerCase().includes(term.toLowerCase()))));
  ...
  const handleDelete = async (id) => {
    const result = await Swal.fire({
      title: 'Delete this task?', text: "This action cannot be undone.", icon: 'warning',
      showCancelButton: true, confirmButtonColor: '#d33', cancelButtonColor: '#3085d6', confirmButtonText: 'Yes, delete it',
    });
    if (!result.isConfirmed) return;
    try { await AdminService.deleteTask(id); await fetchTasks(); Swal.fire('Deleted!', 'The task was successfully deleted.', 'success'); }
  ...
  const applyFilters = () => {
    ...
    if (startDateRange.startDate && startDateRange.endDate) {
      const start = new Date(startDateRange.startDate);
      const end = new Date(startDateRange.endDate); end.setHours(23,59,59,999);
      filteredTasks = filteredTasks.filter(task => { const t = new Date(task.startDate); return t >= start && t <= end; });
    }

    // Due Date Filter
    if (dueDateRange.startDate && dueDateRange.endDate) {
      const start = new Date(dueDateRange.startDate);
      const end = new Date(dueDateRange.endDate); end.setHours(23,59,59,999);
      filteredTasks = filteredTasks.filter(task => { const t = new Date(task.dueDate); return t >= start && t <= end; });
    }

    // Client Filter
    if (clientFilter) filteredTasks = filteredTasks.filter(task => task.companyName === clientFilter);

    // Search Filter
    if (searchQuery.trim() !== '') {
      const term = searchQuery.toLowerCase();
      const fields = ['title','description','employeeName','taskStatus','priority','companyName'];
      filteredTasks = filteredTasks.filter(task => fields.some(f => (task[f] || '').toLowerCase().includes(term)));
    }
  ...
  const formatRangeDisplay = (start, end) => !start||!end ? 'Select date range' : (start.toDateString()===end.toDateString()? start.toLocaleDateString('en-US',{month:'short',day:'numeric'}) : `${start.toLocaleDateString('en-US',{month:'short',day:'numeric'})} - ${end.toLocaleDateString('en-US',{month:'short',day:'numeric'})}`);

  const handleClearFilters = () => {
    setStartDateRange({ startDate:null,endDate:null,key:'selection' });
    setDueDateRange({ startDate:null,endDate:null,key:'selection' });
    setSearchQuery(''); setClientFilter(null); setTasks(allTasks);
  };

  const toggleViewMode = () => setViewMode(viewMode==='grid'?'list':'grid');
  const handleClientSelect = (client) => { setClientFilter(client); setShowClientDropdown(false); };
  const clearClientFilter = () => setClientFilter(null);

  const groupedTasks = tasks.reduce((groups, task) => { const dateKey = task.startDate ? new Date(task.startDate).toISOString().slice(0,10) : 'No Date'; if (!groups[dateKey]) groups[dateKey]=[]; groups[dateKey].push(task); return groups; }, {});
  const sortedDateKeys = Object.keys(groupedTasks).sort((a,b)=>a==='No Date'?1:b==='No Date'?-1:new Date(a)-new Date(b));

  const getStatusColor = (status) => {
    switch((status||'').toLowerCase()){case 'completed':return'bg-green-100 text-green-800';case 'in progress':return'bg-yellow-100 text-yellow-800';case 'pending':return'bg-blue-100 text-blue-800';default:return'bg-gray-100 text-gray-800';}
  };
