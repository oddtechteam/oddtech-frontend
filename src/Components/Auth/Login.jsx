...
const Login = () => {
  ...
  const LOGIN_URL = `${BASE_URL}${import.meta.env.VITE_API_LOGIN}`;
  ...
  useEffect(() => {
    if (threeContainerRef.current && !showFaceLogin) {
      ...
      const animate = () => {
        ...
        cubes.forEach(cube => { cube.rotation.x += 0.005; cube.rotation.y += 0.003 });
        controls.update();
      ...
      return () => {
        ...
        if (threeContainerRef.current) threeContainerRef.current.removeChild(renderer.domElement);
      };
    }
  }, [showFaceLogin]);
  ...
  const loginUser = async (credentials) => {
    try {
      ...
      else throw new Error('Error setting up request');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const onSubmit = async (data) => {
    try {
      ...
      if (response.userId) {
        const userObj = { id: response.userId, role: response.userRole, jwt: response.jwt, email: data.email };

        if (response.userRole === 'ADMIN') {
          localStorage.setItem('user', JSON.stringify({ id: userObj.id, role: userObj.role }));
          localStorage.setItem('token', userObj.jwt);

          gsap.to(".min-h-screen", { opacity: 0, duration: 0.5, onComplete: () => navigate('/admindashboard') });
          ...
          gsap.to(rightPanelRef.current, {
            ...
            onComplete: () => {
              ...
              gsap.fromTo(rightPanelRef.current, { opacity: 0, scale: 0.9 }, { opacity: 1, scale: 1, duration: 0.6 });
            }
          });
        }
      } else {
        ...
        gsap.fromTo(leftPanelRef.current, { x: 0 }, { x: 10, duration: 0.08, repeat: 5, yoyo: true, ease: "power1.inOut" });
      }
    } catch (error) {
      setIsError(true);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-tr from-blue-50 to-blue-100 overflow-hidden">
      ...
      <div className="flex flex-col md:flex-row bg-white rounded-2xl shadow-2xl overflow-hidden max-w-4xl w-full mt-10 md:mt-20">
        <div className="w-full md:w-1/2 p-10" ref={leftPanelRef}>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div className="flex items-center justify-between text-sm mt-2">
              ...
              <button type="button" className="hover:underline" style={{ color: '#00A3E1' }}>Forgot password?</button>
            </div>

            <Button type="submit" variant="contained" fullWidth sx={{ backgroundColor: '#00A3E1', '&:hover': { backgroundColor: '#0081B4' }, transition: 'all 0.3s ease' }} disabled={isLoading}>
          ...
          <CardActions>
            <Button fullWidth onClick={() => { gsap.to(".min-h-screen", { opacity: 0, duration: 0.5, onComplete: () => navigate('/contactus') }); }} disabled={isLoading} sx={{ fontSize: 'small', color: '#00A3E1', '&:hover': { backgroundColor: 'transparent', color: '#0081B4', textDecoration: 'underline' } }}>
        ...
        {/* Right Panel */}
        <div className="md:flex w-full md:w-1/2 text-white items-center justify-center p-10 relative bg-gradient-to-br from-blue-800 via-blue-600 to-blue-400" ref={rightPanelRef}>
          <div ref={threeContainerRef} className="absolute inset-0 w-full h-full" style={{ zIndex: 0 }} />
            ...
            {showFaceLogin && loggedUser?.role === 'EMPLOYEE' ? (
              <FaceLogin
                ...
                onSuccess={() => {
                  ...
                  gsap.to(".min-h-screen", { opacity: 0, duration: 0.5, onComplete: () => navigate('/employeedashboard') });
                }}
                onFailure={() => {
                  setIsError(true);
                  setSnackbarMessage('Face not recognized');
                  setOpenSnackbar(true);
                  gsap.to(rightPanelRef.current, { opacity: 0, scale: 0.9, duration: 0.4, onComplete: () => {
