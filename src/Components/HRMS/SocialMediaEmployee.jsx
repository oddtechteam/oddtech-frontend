...
const EmployeePortal = () => {
  ...
  const handleAddNote = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8080'}/api/note/createnote`,
        {
          method: "POST",
          headers: {
  ...
  const handleAddIssue = async () => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8080'}/api/issue/createissue`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
  ...
  const fetchIssues = async () => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8080'}/api/issue/allissues`, {
        method: "GET",
        headers: {
          "Authorization": `Bearer ${token}`
        }
  ...
  const handleAddPost = async () => {
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8080'}/api/post/createpost`, {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${token}`,
        },
  ...
  const fetchPosts = async () => {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8080'}/api/post/allpostbydesc`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  const likePost = async (postId) => {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8080'}/api/post/like/${postId}`, {
        method: "PUT",
        headers: {
          Authorization: `Bearer ${token}`,
        }
  ...
  const fetchNotes = async () => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8080'}/api/note/allnotes`, {
        method: "GET",
        headers: {
          "Authorization": `Bearer ${token}`
        }
